...
public class Selection
{
	...
	//*****TODO********** I (David) need to convert this to use the list natively
	...
	*/
	public Selection(List<int[]> facts, Random rand)
	{
		this.rand = rand;
		factArray = facts.toArray(new int[facts.size()][facts.get(0).length]);
		evalArray = new float[factArray.length];
		indexArray = new float[factArray.length];
		...
		for (int i = 0; i< factArray.length; i++)
		{
			tempValue = Driver.eval.getValue(factArray[i]);
